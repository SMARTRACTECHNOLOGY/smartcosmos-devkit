...
function http_get_succeeds() {
  ...
  if [ ${attempt} -eq ${MAX_ATTEMPTS} ] && [ ${started} = false ]
  then
    return 1
  else
    return 0
...
function container_is_up() {
  ...
  if [ ${container_count} -eq ${container_up_count} ]
  then
    return 0
  else
    return 1
...
function configserver() {
  ...
  if http_get_succeeds ${CONFIG_SERVER_STATUS_URL}
  then
...
function services() {
  ...
  if ! container_is_up "edge-user-devkit|user-details-devkit|auth-server"
  then
    sleep 60s & spinner
  fi
  ...
  if ! container_is_up "ext-relationships|ext-metadata|ext-things|edge-things"
  then
    sleep 90s & spinner
  fi
...
function main() {
  ...
  if http_get_succeeds "http://localhost:8080"
  then
